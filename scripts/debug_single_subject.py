...
def find_perfectly_correlated_pairs(corr_matrix: pd.DataFrame, threshold: float = 0.999):
...
def main():
    ...
    if not problematic_pairs.empty:
        ...
        log.info("No se encontraron pares de ROIs con correlación perfecta. La singularidad puede deberse a otras razones.")
    print("-" * 70)

    # --- VISUALIZACIÓN ---

    # 1. Heatmap Estático (para la tesis)
    ...
    plt.xlabel('Índice de ROI')
    plt.ylabel('Índice de ROI')
    ...
    # 2. Heatmap Interactivo (para exploración y suplementos)
    if PLOTLY_AVAILABLE:
        log.info("Generando heatmap interactivo con Plotly...")
        fig = px.imshow(corr_matrix,
                        color_continuous_scale='Viridis',
                        zmin=-1, zmax=1,
                        title=f'Heatmap de Correlación Interactivo para Sujeto {SUBJECT_ID_TO_DEBUG}',
                        labels=dict(x="Índice de ROI", y="Índice de ROI", color="Correlación"))
